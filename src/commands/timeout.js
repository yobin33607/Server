...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (target.roles.highest.position >= message.member.roles.highest.position && message.author.id !== message.guild.ownerId) {
      return message.reply('You cannot timeout a member with equal or higher role than you.');
    }
    ...
    modLogs.add(message.guildId, target.id, message.author.id, 'Timeout', reason);
